...
from cleverhans.future.torch.attacks import fast_gradient_method, projected_gradient_descent, spsa
...
def main(_):
  ...
  for x, y in data.test:
    ...
    _, y_pred_spsa = net(x_spsa).max(1)  # model prediction on PGD adversarial examples
  ...
  print('test acc on SPSA adversarial examples (%): {:.3f}'.format(report.correct_spsa / report.nb_test * 100.))
